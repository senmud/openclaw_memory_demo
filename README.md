...
- **session 命名规则**：`memory_{userId}:{channelId}:{threadId}.md`
...
在你的 `openclaw.json` 中启用插件并配置模式（示例）：
...
- **session 模式**：`memoryMode: "session"`  
  ...
  - `userId/channelId/threadId` 由调用工具时传入参数决定
...
- **`markdown_memory_add`**
  ...
  - **参数**：
    ...
    - `userId?: string` / `channelId?: string` / `threadId?: string`：
      - 当 `memoryMode === "session"` 时三者都必填，用作文件名的一部分实现隔离
...
- **Markdown 文件实现 session 级别存储**  
  - 使用 `~/.openclaw/markdown-memory/sessions/memory_{userId}:{channelId}:{threadId}.md` 作为 session 级文件。
...
- **二者互斥**  
  - 插件内部根据 `memoryMode` 决定路径逻辑：
    ...
    - `session` 模式下强制要求 `userId/channelId/threadId`，并写入各自文件  
  - 不存在同时激活两种模式的代码路径。
